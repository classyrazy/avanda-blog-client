...
async function handleFormRequest() {
    ...
    toServerForm()
    try {
        let req = new Graph().service('User/signin')
        let user = await req.post(ServerForm)
        let data = user.getData()
        if (user) {
            localStorage.setItem('user-token', user.getData().token);
            createToast(ToastText, { text: "Login Successful redirecting...." })
        ...
        console.log(form)
        createToast(withProps(ToastText, { text: error.getMsg() }), {
            showIcon: "true",
            swipeClose: "true",
            type: "danger",
            transition: "bounce",
